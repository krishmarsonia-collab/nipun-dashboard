...
export default function BlockResultPage({ blockComparisons, scope }) {
  const resultRows = useMemo(() => buildClusterResults(blockComparisons), [blockComparisons]);
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-sky-800/80 mt-1 leading-relaxed">
              For each block, the school's self-reported Nipun % is the official result — unless
              it reads more than 5 percentage points higher than the independent verifier sample,
              aggregated across the whole block, in which case the verifier's numbers become
              the official result for that block instead. A verifier sample reading higher than
            ...
            <p className="text-xs text-sky-700/70 mt-3">
              ...
              <span className="font-semibold text-sky-900">{counts.total.toLocaleString()}</span> blocks use
      ...
      <ResultClusterList rows={resultRows} entityLabel="Block" showLocationMeta={false} />
    </div>
  );
}
