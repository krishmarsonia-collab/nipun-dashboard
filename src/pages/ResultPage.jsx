...
export default function ResultPage({ clusterComparisons, scope }) {
  const resultRows = useMemo(() => buildClusterResults(clusterComparisons), [clusterComparisons]);
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-sky-800/80 mt-1 leading-relaxed">
              For each cluster, the school's self-reported Nipun % is the official result — unless
              it reads more than 5 percentage points higher than the independent verifier sample,
              in which case the verifier's numbers become the official result for that cluster
              instead. A verifier sample reading higher than the school never overrides it, by any margin.
            </p>
            <p className="text-xs text-sky-700/70 mt-3">
              ...
              <span className="font-semibold text-sky-900">{counts.total.toLocaleString()}</span> clusters use
      ...
      <ResultClusterList rows={resultRows} />
    </div>
  );
}
